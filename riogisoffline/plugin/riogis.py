...
class RioGIS:
    ...
    def show_selected_feature(self, data):
        ...
        length = f'{data["length"]:.2f} m' if data["length"] else ""

        text = f'Ledning valgt: LSID: <strong>{data["lsid"]}</strong> (fra PSID {data["from_psid"]} til {data["to_psid"]}), Gate: <strong>{streetname}</strong>, Type: <strong>{fcode}</strong>, Dim: <strong>{data["dim"]} mm</strong>, Materiale: <strong>{data["material"]}</strong>, Lengde: <strong>{length}</strong>'
